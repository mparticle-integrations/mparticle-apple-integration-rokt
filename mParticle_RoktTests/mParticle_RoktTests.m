...
- (void)testGlobalEvents_Success {
    Class mockRoktClass = OCMClassMock([Rokt class]);

    __block BOOL callbackCalled = NO;
    __block MPRoktEvent *receivedEvent = nil;

    // Mock the Rokt SDK class method and simulate triggering the callback with a mock event
    OCMStub([mockRoktClass globalEventsOnEvent:[OCMArg any]]).andDo(^(NSInvocation *invocation) {
        // Get the callback block from the invocation
        void (^onEventCallback)(RoktEvent *) = nil;
        [invocation getArgument:&onEventCallback atIndex:2]; // Index 2 is the first parameter (onEvent)

        // Create a dummy ShowLoadingIndicator for testing
        InitComplete *event = [InitComplete alloc];
    ...
    // Execute the method under test
    MPKitExecStatus *status = [self.kitInstance globalEvents:^(MPRoktEvent * _Nonnull event) {
        callbackCalled = YES;
        receivedEvent = event;
    }];

    // Verify the Rokt SDK method was called
    OCMVerify([mockRoktClass globalEventsOnEvent:[OCMArg any]]);
    ...
    XCTAssertTrue(callbackCalled);
    XCTAssertNotNil(receivedEvent);
    XCTAssertTrue([receivedEvent isKindOfClass:[MPRoktInitComplete class]]);

    [mockRoktClass stopMocking];
}

- (void)testGlobalEvents_MappingReturnsNil {
    Class mockRoktClass = OCMClassMock([Rokt class]);
    ...
    OCMStub([mockRoktClass globalEventsOnEvent:[OCMArg any]]).andDo(^(NSInvocation *invocation) {
        // Get the callback block from the invocation
        void (^onEventCallback)(RoktEvent *) = nil;
        [invocation getArgument:&onEventCallback atIndex:2];
